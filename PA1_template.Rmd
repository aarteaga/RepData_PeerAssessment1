...
The data are loaded in a zip file that has to be decrompressed. The only prepocessing done at this step is to convert date data to date format. The output in R is a dataframe with 3 variables: 
 - date: the date of data collection in R date format
 - interval: The time of the data adquisition in 5 minutes interval time
 - steps: the number of steps measured in the interval.
...
Nest analsysis consists of checking differences among intervals of measurement, so about moments of the day. A time series plot is used for that purpose.
```{r}
...
hist( stepsperday_noNa , main="Steps per Day" , 
     xlab= "Number of Steps", ylab = "Number of Days")
```
